...
app.post("/printdelivery", async (req, res) => {
  try {
    ...
    // Esta ruta solo debe manejar tickets individuales cuando se especifica IP
    // Si no se especifica IP, no hacer nada (los formularios manejan todo)
    res.json({
      success: true,
      message: "No se especificó IP para impresión individual",
      results: ["No procesado - usar lógica de formularios"]
    });
  } catch (err) {
