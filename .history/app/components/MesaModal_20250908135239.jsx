...
export default function ModalMesa({ mesa, onClose, refetch }) {
  ...
  const imprimirTicket = async (productos, numeroMesa, orden, hora, fecha) => {
    // Separar productos por categoría
    const productosBrasas = productos.filter(p => p.categoria?.toLowerCase() === "brasas");
    const productosNoBrasas = productos.filter(p => p.categoria?.toLowerCase() !== "brasas");
    
    const tieneBrasas = productosBrasas.length > 0;
    const tieneNoBrasas = productosNoBrasas.length > 0;

    const enviarAImpresora = async (items, ip, tipoImpresora) => {
      if (items.length === 0) return;
      
      console.log(`📤 Enviando a ${tipoImpresora}:`, { ip, items: items.length, mesa: numeroMesa });
    ...
    console.log("🔍 Debug impresión mixta:", { 
      tieneBrasas, 
      tieneNoBrasas, 
    ...
    // Generar tickets separados según el tipo de productos
    if (tieneBrasas && tieneNoBrasas) {
      // Productos mixtos: 2 tickets separados
      console.log("🔥🍽️ Productos mixtos: enviando 2 tickets separados");
      ...
      // Solo no brasas: 1 ticket en cocina
      console.log("🍽️ Solo no brasas: enviando a cocina (192.168.1.100)");
      await enviarAImpresora(productosNoBrasas, "192.168.1.100", "cocina");
    }
  };
