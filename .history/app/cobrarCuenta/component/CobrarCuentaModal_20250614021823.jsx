...
export default function CobrarCuentaModal({
  ...
  const confirmarPago = async () => {
    ...
    // 👉 ACA: Registramos el ingreso en la caja si es efectivo
    if (metodo === "Efectivo") {
      try {
        await fetch("/api/cobro-efectivo", {
          ...
          body: JSON.stringify({
            totalPedido: totalFinal, // el total real
            pagoCliente: parseFloat(montoPagado), // cuánto pagó el cliente
          }),
        });
      } catch (err) {
    ...
    // Liberamos la mesa como ya lo hacías antes
