...
export default function CajaRetiro() {
  ...
  useEffect(() => {
    ...
    const fechaArgentina = new Date(ahoraArgentina);
    const horaActual = fechaArgentina.getHours();
    const minutosActual = fechaArgentina.getMinutes();

    if (horaActual > 23 || (horaActual === 23 && minutosActual >= 30)) {
